...
non-bindless DecalV5, kill-counter DecalCounterV5, and DecalGlowV5 browser
gates use its one-type batch-map path, with the decal fixtures proving that
numeric batch type `1` stays caller-selected. The duck-typed boundary is
shaped for future `runtime-trinity` `Tr2RenderBatch`,
